...
class Message {
 public:
  ...
  int dst;
  int timeout;  // measured by circle ,the time limit of message waits for a
                // channel
  int begintrans;  // when a message is generated ,it needs some time until
                   // transmitting,begintrans record this.
  int step;            // how many steps this message has walked;
  bool active;         // check this message whether consumed or arrive at dst
  NodeInfo* routpath;  // : the ith flit now at routpath[i].node and take
                       // routpath[i].buffer
  int count;           // the total time a message  consumed
  bool releaselink;  // if the tail of a message shifts , the physical link the
                     // message occupied should release.

  bool turn;  //  used in bubble flow control, if true, then the request escape
              //  channel need 2 buffers.
